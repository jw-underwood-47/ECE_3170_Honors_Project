 ...
 * these groups defined but not the rest */
    ...
    #define INVERTING 2
#endif
void do_clustered_error(uint64_t *target, results_t *results);
void do_more_burst_errors(uint64_t *target, results_t *results);
void set_error_spots(uint64_t* target, results_t *results);
void do_burst_error(uint64_t *target, results_t *results);
...
/* NOTE: the per-message "original" value and its encoded/corrupted
 * representation are no longer globals -- they live in message_t
 * (see struct_def.h) and get passed around explicitly instead. */
...
/*
 * hamming(7, 4) encoding of the given original value;
 * assumes given target of two uint64_ts
 */
void hamming_encode_74(uint64_t *target, uint64_t original){
    for (int i = 0; i < 16; i++){
